...
Chaque module doit tenir dans son budget de commandes top-level ci-dessous. Tout le reste va dans un groupe (`slash: { group }`), en général nommé comme le module ou un alias court. Les groupes acceptent jusqu'à 25 sous-commandes (+ sous-groupes).
`admin`: help, embed, module, settings, prefix, bot(g) · `moderation`: ban, unban, kick, timeout, untimeout, purge, lock, unlock, slowmode, warn(g), case(g), role(g), mod(g) · `automod`: automod(g) · `antiraid`: antiraid(g), verify · `sanctions`: sanction(g), jail, tribunal(g) · `logs`: logs(g) · `welcome`: welcome(g) · `invites`: invites(g) · `stats`: stats(g) · `tickets`: ticket(g), ticketadmin(g) · `suggestions`: suggest, suggestion(g) · `starboard`: starboard(g) · `quotes`: quote(g) · `reactionroles`: reactionroles(g) · `economy`: balance, daily, work, pay, shop(g), inventory, eco(g), market(g) · `casino`: blackjack, roulette, slots, bet(g), casino(g) · `rpg`: fight(g), job(g), pet(g), rob · `leveling`: rank, leaderboard, xp(g), achievements · `birthday`: birthday(g) · `utility`: userinfo, serverinfo, avatar, translate, weather, convert(g), util(g), afk, snipe · `tools`: tools(g) · `fun`: fun(g), meme(g), image(g), 8ball, game(g) · `giveaways`: giveaway(g) · `polls`: poll(g) · `reminders`: remind(g) · `customcommands`: tag(g), autoresponder(g) · `announcements`: announce(g) · `feeds`: feed(g), stream(g) · `music`: play, skip, stop, pause, resume, queue, nowplaying, volume, loop, shuffle, music(g), radio(g), filter(g), record(g), blindtest(g) · `tempvoice`: voice(g) · `sysadmin`: sys(g), docker(g), proxmox(g), wol · `network`: net(g) · `backup`: backup(g) · `integrations`: hooks(g), integration(g), fetch · `gaming`: mc(g), fivem(g), gaming(g), lfg(g) · `ai`: ask, ai(g).
